...
### M19 — Escritura asistida y primer agente (Programador) ✅ completado (2026-07-06)
...
**Entregado:** el Chat gana un segundo modo explícito, **"Proponer cambios"** (junto al "Preguntar" de M18), elegido por el usuario, no inferido por el modelo. En ese modo, `ai_propose_change` (`editor/src-tauri/src/ai.rs`) pide al proveedor un único objeto JSON (`ChangeProposal`: altas/parches/bajas de entidades + scripts `.rhai` nuevos) — con `format:"json"` de Ollama como ayuda para modelos locales, y extracción tolerante del primer `{...}` balanceado de la respuesta para el resto de casos (prosa/fences alrededor). La propuesta se **valida en Rust contra los tipos reales del formato** (`aigs_project::EntityNode`/`Components`, ya derivan `Deserialize`): cualquier componente inventado o mal tipado se rechaza ahí mismo; las referencias a assets (`sprite`/`script`/`particles`) se comprueban contra los assets ya existentes más los que la propia propuesta declara; cada script nuevo se **compila de verdad** con `rhai::Engine::compile` antes de mostrarse (un script con error de sintaxis nunca llega a la tarjeta de "Aplicar"); los ids de entidad nuevos/actualizados/eliminados se comprueban contra los ya existentes en la escena (todo o nada: la propuesta entera se acepta o se rechaza con un motivo claro, nunca una aplicación parcial). El manifiesto `scripting-api.json` (M12) se embebe en el *system prompt* con `include_str!` como contrato de funciones válidas. **Aplicar** una propuesta reutiliza `UPDATE_DOCUMENT`/el historial de undo ya existentes desde M3 (ningún mecanismo de persistencia nuevo): escribe los scripts nuevos en `assets/` vía el comando `write_script_asset` (misma convención que `import_asset`), fusiona los parches de componentes campo a campo (`mergeComponentsPatch` en `document.ts`, para que un parche parcial como `{transform2d:{x:5}}` no borre `y`/`rotation`) y aplica altas/bajas con los helpers ya existentes de `document.ts` (`insertEntity`/`patchComponents`/`removeEntity`) — deshacer un cambio de la IA es `Ctrl+Z`, sin código especial. **Verificado de punta a punta contra un Ollama real** (`qwen2.5-coder:7b`, ~7 min de generación en CPU): pedido "añade una moneda en (50,50) usando el sprite 'hero' existente" produjo un JSON válido, con el asset correcto y sin inventar nada, que pasó la validación de Rust sin cambios — prueba conservada como test `#[ignore]` (`propose_change_end_to_end_with_real_ollama`, se ejecuta a mano con `cargo test -- --ignored`, no en el suite normal por su duración). El resto de la validación (9 casos: propuesta válida, extracción tolerante de prosa/fences, asset desconocido, script que no compila, id de entidad duplicado/desconocido, ruta de archivo insegura) corre como tests unitarios normales, sin red. `cargo fmt`/`clippy`/`test` limpios en `editor/src-tauri` y en el workspace principal (sin cambios en `aigs-project`/`aigs-runtime`, solo se reutilizan sus tipos ya existentes); build (`npm run build`) y arranque (`npm run tauri dev`) verificados, recorrido de clics en la UI real **no** verificado (sin automatización de pantalla en este entorno).

---

### M20 — Agentes especializados ✅ completado (2026-07-07)
...
**Alcance reducido a 6 agentes reales** (decisión registrada en `docs/arquitectura.md`): Arquitecto, Diseñador de niveles, Programador (ya existente desde M19), Física, Audio y Animador. **"Optimización" queda diferido** — no hay ningún dato de perfilado (tamaño de assets, FPS reales) que llegue al modelo todavía, así que un agente de "optimización" no tendría base real sobre la que proponer nada. **"Animador" se reduce a conectar animaciones ya existentes** vía el componente `animator` (sin autoría de keyframes/tracks nuevos — eso sigue siendo del Timeline panel, manual); se valida que cada nombre de animación referenciado exista de verdad en la escena.

**Entregado:** un tercer modo de Chat, **"Orquestar agentes"** (junto a "Preguntar" de M18 y "Proponer cambios" de M19). Nuevo módulo `editor/src-tauri/src/agents.rs`: el **Arquitecto planifica** (`{"summary", "steps": [{"agent", "instruction"}]}`, máximo 8 pasos) con el mismo mecanismo de JSON validado que M19; cada paso se ejecuta **secuencialmente** contra el *prompt* de su especialista (`build_scoped_agent_prompt`) y se valida con la **misma** `parse_and_validate_proposal` de M19, ahora parametrizada por un `AgentKind` opcional que impone una **lista blanca de componentes por agente** (comprobada en Rust, no solo pedida en el *prompt*): Arquitecto → `transform2d`/`sprite`; Diseñador de niveles → + `collider2d`; Programador → `script`/`behaviors`; Física → `rigidbody2d`/`collider2d`/`gravity` de escena; Audio → `behaviors`/`music` de escena; Animador → `animator`. Nuevo campo `scene_patch: {gravity?, music?}` en `ChangeProposal` para que Física/Audio puedan tocar campos de la propia escena (M19 solo cubría entidades). Los ids/assets/animaciones que un paso crea se acumulan para los pasos siguientes (un paso posterior puede referenciar una entidad que uno anterior acaba de crear dentro del mismo plan) y todo el plan se rechaza si cualquier paso falla la validación (mismo principio "todo o nada" de M19, identificando qué paso/agente falló). El resultado final es un `ChangeProposal` idéntico en forma al de M19, así que el frontend reutiliza sin cambios la tarjeta de propuesta y el aplicar/deshacer ya construidos (solo se sumó el render de `scene_patch` y su aplicación a la escena). **Verificado de punta a punta contra un Ollama real** (`qwen2.5-coder:7b`, 3 llamadas — planificación + 2 pasos, ~6 min): una primera ejecución real expuso un caso genuino de validación funcionando como se diseñó (el paso de Diseñador de niveles propuso `"shape": "rectangle"`, valor que el formato no admite — solo `"box"`/`"circle"` — y la orquestación completa se rechazó con un mensaje señalando el paso y el motivo exactos); el *prompt* de cada agente se ajustó para deletrear los valores exactos de los enums (`shape`, `body`) tras este hallazgo, y una segunda ejecución real confirmó que el plan de dos pasos (Arquitecto coloca la plataforma, Diseñador de niveles le añade un colisionador de caja) se genera, valida y fusiona correctamente. Conservado como test `#[ignore]` (`orchestrate_end_to_end_with_real_ollama`) por su duración. 4 tests unitarios más (sin red) cubren planificación vacía/con demasiados pasos/agente desconocido/envuelta en prosa, y 4 tests de alcance por agente (componente fuera de alcance rechazado/aceptado, gravedad de escena desde un agente que no es Física rechazada, animación desconocida rechazada). `cargo fmt`/`clippy`/`test` limpios en `editor/src-tauri` y en el workspace principal; build (`npm run build`) y arranque (`npm run tauri dev`) verificados, recorrido de clics en la UI real no verificado (sin automatización de pantalla en este entorno).

---

### M21 — Generación de juegos completos ✅ completado (2026-07-07)
...
**Alcance real, honesto** (registrado en `docs/arquitectura.md`): "juego completo" significa **estructura completa** (escenas, entidades, física, comportamientos, scripts, música/efectos ya importados), no arte/audio nuevo — no hay ningún modelo de generación de imágenes/audio integrado en este proyecto. Si la instrucción necesita un asset que el proyecto no tiene, ese paso falla la validación de referencias a assets exactamente igual que en M19/M20 — el usuario debe importar al menos arte de marcador de posición antes de pedirle a la IA que construya el juego alrededor.

**Entregado:** un cuarto modo de Chat, **"Generar juego"**. Nuevo rol **"Productor"** en `editor/src-tauri/src/agents.rs`, un nivel de planificación por ENCIMA del Arquitecto de M20: decide qué **escenas** hace falta (`{"summary","scenes":[{"name","is_new","goal"}]}`, máximo 6), cada una marcada como la escena ya abierta en el editor (`is_new: false`, como mucho una en el plan) o una escena nueva a crear (`is_new: true`). Por cada escena del plan se reutiliza el **mismo** `orchestrate()` de M20 sin ningún cambio (Arquitecto → especialistas, alcance por lista blanca de componentes, todo o nada) — una escena nueva arranca sin entidades/animaciones propias, la escena actual conserva las que ya tenía. Esto cierra una brecha real que había quedado abierta en M20: su propio ejemplo de entregable ("crea un segundo nivel más difícil") necesitaba crear una escena, algo que `orchestrate()` nunca soportó por sí solo. El resultado (`ProjectProposal`: un `ChangeProposal` validado por escena) se aplica **de forma atómica**: un único `UPDATE_DOCUMENT` escribe todas las escenas nuevas/actualizadas a la vez, así que deshacer un juego completo generado por la IA es un solo `Ctrl+Z`, igual que cualquier otro cambio de la IA. **La iteración conversacional no necesitó código nuevo**: verificado por inspección del propio código (no por un recorrido de clics real, sin automatización de pantalla en este entorno) — `ChatPanel`'s `submit()` lee `state.loaded`/`currentScene` del store de React en cada envío, y el store ya se actualiza de forma síncrona tras aplicar una propuesta (`UPDATE_DOCUMENT`), así que el siguiente mensaje del chat ya ve el resultado aplicado como parte de su contexto — pedir "hazlo más rápido" es simplemente otra petición de "Proponer cambios"/"Orquestar agentes" sobre lo que ya existe.

**Verificado de punta a punta con Ollama real** (`qwen2.5-coder:7b`), con un caso de estudio real: "un juego con dos escenas, un menú con el sprite del héroe centrado y un nivel con una plataforma estática usando el mismo sprite". Cada intento son varias llamadas secuenciales a un modelo local en CPU (Productor + una orquestación Arquitecto/especialistas completa por escena), así que cada ejecución tardó entre 5 y 14 minutos. **Costó cinco intentos reales llegar a un resultado válido**, y los cuatro primeros expusieron, cada uno, un fallo de validación genuino y distinto — la prueba más clara de que "el *prompt* orienta, el backend decide" funciona de verdad: (1) un campo de texto obligatorio en `null` en vez de un valor u omitirlo; (2) un script con su contenido anidado como objeto en vez de declararlo en `scripts` y referenciarlo por id de texto; (3) y (4) un especialista reutilizando el id de una entidad que otro paso del mismo plan ya había creado, intentando recrearla con `entities_to_add` en vez de modificarla con `entities_to_update`. Cada hallazgo se corrigió aclarando el *prompt* compartido de todos los agentes (beneficia a M19/M20 también, no solo a M21) — ninguno reveló un error de la validación en sí, que rechazó cada propuesta inválida con un mensaje señalando exactamente el paso y el motivo, nunca aplicando nada parcial. El quinto intento generó, validó y fusionó las dos escenas correctamente: una jerarquía padre-hijo razonable en el menú (una entidad "centro" con el sprite del héroe como hijo) y la plataforma del nivel construida en tres pasos coherentes (Arquitecto crea la entidad y le añade el sprite, Diseñador de niveles le añade el colisionador — los dos últimos correctamente como `entities_to_update` sobre la misma entidad, no como altas duplicadas). Conservado como test `#[ignore]` (`generate_project_end_to_end_with_real_ollama`) dada su duración. 4 tests unitarios más (sin red) cubren la validación del plan del Productor (vacío, demasiadas escenas, nombre de escena inseguro, plan válido). `cargo fmt`/`clippy`/`test` limpios en `editor/src-tauri` y en el workspace principal; build (`npm run build`) y arranque (`npm run tauri dev`) verificados, recorrido de clics en la UI real no verificado.

**Cierra la Fase 4 con M18-M21 completos** (M22-M25 quedan para la Fase 5, Ecosistema). Release **0.4.0** publicado siguiendo el mismo procedimiento de 0.1/0.2/0.3.
