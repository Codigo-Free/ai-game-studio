...
- **Presupuesto de tamaño (M17):** el `.wasm` se compila con `opt-level = "s"`, `lto = true` y `strip = true`, y luego se pasa por `wasm-opt -Oz` (ver CI, job *Web player*) antes de considerarse el artefacto definitivo — reduce el binario sin tocar el código del motor.
...
- Exportable también desde el botón **⬇ Exportar** del editor (desplegable Desktop/Web/Android, M17).
- Limitaciones conocidas: sin `save.json` todavía (ver SPEC.md, sección de persistencia).
...
- **Presupuesto de tamaño (M17):** el build `--release` usa `opt-level = "s"`, `lto = true` y `strip = true` en el `.so` — el debug build (el que produce el editor y `aigs export` sin `--release`) no lleva esta optimización, es más grande y más rápido de compilar a propósito.
- Exportable también desde el botón **⬇ Exportar** del editor (desplegable Desktop/Web/Android, M17) — el editor solo compila un debug build; el release firmado sigue siendo solo CLI con `--release`.
- Limitaciones conocidas: build de depuración por defecto (release necesita un keystore propio configurado en `[package.metadata.android.signing.release]`); las texturas no se recargan si la app se suspende y reanuda (ver arquitectura.md); sin `save.json` todavía.
