...
| 2026-07-07 | **M20 (agentes especializados): orquestación en dos fases determinista (Arquitecto planifica → especialistas ejecutan en secuencia → se fusiona en un único `ChangeProposal`)**, no un agente conversacional con bucle de herramientas | Mismo principio que M19: mantener `Provider::chat` genérico para Ollama/Claude sin implementar *function calling* específico de cada proveedor. El Arquitecto responde un plan (`{"summary", "steps": [{"agent", "instruction"}]}`) con el mismo mecanismo de JSON validado que ya existe; cada paso se ejecuta contra el prompt/alcance de su especialista y se valida con la **misma** `parse_and_validate_proposal` de M19 (ahora parametrizada por un `AgentKind` opcional); el resultado se fusiona en un `ChangeProposal` idéntico en forma al de M19, así que el frontend reutiliza sin cambios la tarjeta de propuesta y el aplicar/deshacer ya construidos. |
| 2026-07-07 | **M20: alcance reducido a 6 agentes reales (Arquitecto, Diseñador de niveles, Programador, Física, Audio, Animador)**, no los 8 de la tabla de `ia.md` | "Diseñador UI" no aparece en la lista de tareas de M20 en `plan.md`. "Optimización" se descarta explícitamente para este hito: no hay ningún dato de perfilado (tamaño de assets, FPS reales) que llegue al modelo todavía, así que un agente de "optimización" no tendría ninguna base real sobre la que proponer cambios — construirlo ahora sería fingir una capacidad que no existe, contrario a la práctica de este proyecto de verificar antes de afirmar. Ambos quedan documentados como diferidos, no implementados a medias. |
| 2026-07-07 | **M20: "Animador" solo conecta animaciones ya existentes** (componente `animator: {initial, states, transitions}` referenciando nombres de `scene.animations` ya creados), **no autoría de keyframes/tracks nuevos** | Autoría de tracks/keyframes ya tiene una herramienta dedicada y validada desde M4 (el panel Timeline); extender el esquema `ChangeProposal` para que un modelo genere keyframes por texto es un salto de alcance mucho mayor que "conectar una máquina de estados a animaciones que el usuario ya diseñó a mano" — y esto último sigue siendo una capacidad real y verificable (se comprueba que cada nombre de animación referenciado exista de verdad en la escena). |
| 2026-07-07 | **M20: el alcance de escritura de cada agente se aplica por lista blanca de claves de componente** (p. ej. Física solo puede tocar `rigidbody2d`/`collider2d`/`gravity` de escena; Audio solo `behaviors`/`music`), comprobada en Rust sobre el mismo `Components` ya validado en M19, no en el *prompt* únicamente | El *system prompt* de cada especialista ya le dice qué puede tocar, pero un modelo puede equivocarse o ignorarlo; validarlo en el backend (rechazar si aparece una clave de componente fuera de la lista permitida) es la misma filosofía que ya rige assets/scripts en M19: el *prompt* orienta, el backend decide. Nuevo campo opcional `scene_patch: {gravity?, music?}` en `ChangeProposal` para que Física/Audio puedan tocar campos de la propia escena (antes M19 solo cubría entidades). |
| 2026-07-07 | **M20: ejecución de pasos secuencial, no paralela**, acumulando los ids de entidad/asset/animación ya conocidos (los propios + los de escena) a medida que cada paso se valida | Permite que un paso posterior referencie una entidad creada por uno anterior dentro del mismo plan (p. ej. Arquitecto crea la entidad, Física le añade un colisionador) sin inventar un mecanismo de dependencias explícito; también evita que dos pasos del mismo plan colisionen sin darse cuenta en el mismo id nuevo. Si cualquier paso falla la validación, se rechaza el plan entero (mismo principio "todo o nada" de M19), identificando qué paso/agente falló. |
| 2026-07-07 | **M20: código nuevo en un módulo separado `agents.rs`**, no ampliando más `ai.rs` (ya en ~700 líneas tras M19) | Mantiene `ai.rs` centrado en el proveedor/protocolo (M18) y la propuesta de un solo agente (M19); la planificación multi-agente, el reparto de alcance por agente y la fusión de propuestas son una responsabilidad distinta que merece su propio archivo, reutilizando `Provider`/`ChatMessage` de `ai.rs` y `EntityNode`/`Components`/`Music`/`Gravity` de `aigs_project` sin duplicarlos. |
