...
- **Alcance de escritura por lista blanca de componentes**, comprobado en Rust (no solo pedido en el *prompt*): cada agente solo puede tocar los componentes que le corresponden — Arquitecto (`transform2d`/`sprite`/`shape`), Diseñador de niveles (+ `collider2d`, también `shape`), Programador (`script`/`behaviors`), Física (`rigidbody2d`/`collider2d`/gravedad de escena), Audio (`behaviors`/música de escena), Animador (`animator`, solo animaciones ya existentes). El componente `shape` (rectángulo/círculo de color plano, sin asset de imagen — ver `docs/arquitectura.md`, 2026-07-10) le da a Arquitecto/Diseñador de niveles una forma de crear indicadores, bloques o placeholders visuales sin depender de un sprite que no existe.
