...
- **Fase 2 (Motor completo) en curso** — plan en `docs/plan.md` (M7–M13). **M7–M12 completados**; siguiente: **M13 (Demo de plataformas + release 0.2)** — cierre de Fase 2; incluye las curvas bezier diferidas.
- M12 entregó: scripting con **rhai 1.25** sandboxeado (sin IO, 200k ops/tick, call levels 32) — asset kind `script` (.rhai) + componente `script: {asset}`; `ScriptHost` compila ASTs una vez y corre una instancia por entidad (`Scope` persistente: `this.campo` entre ticks) con `fn on_start()`/`fn on_update(dt)`; la API lee por snapshot (x/y/rotation/frame, x_of/y_of/distance_to, key_*) y muta por **cola de comandos** (set_pos/move_by/set_rotation/set_frame directos al mundo; goto_scene/play_animation/play_sound/emit_particles al GamePlayer); errores compilación/runtime → warnings y el script se desactiva (failed flag, sin spam). Corre tras behaviors/animators y antes de física (mueve kinemáticos). Editor: import .rhai (📜), sección Script en inspector. Demo: dron patrullero en robot-rescue (~15 líneas).
