...
- **Fase:** 1 (MVP) — **M0–M3 completados; M4 con código completo** (pendiente validación manual del Timeline); siguiente: validar M4 y arrancar **M5 (Escenas y modo Play)**.
...
| 2026-07-03 | **M4 código completo:** evaluación de keyframes (`aigs_anim::sample`), `AnimationPlayback` en runtime (loop/hold, warnings de bind), `aigs run` reproduce animaciones, panel Timeline en el editor (pistas, keyframes arrastrables, easing, scrubbing, play con preview en viewport), semántica de reproducción documentada en la SPEC, animación `float` en loop añadida a hello-world. Spritesheet diferido a Fase 2. | [sdk/aigs-format/SPEC.md](sdk/aigs-format/SPEC.md), [editor/src/panels/Timeline.tsx](editor/src/panels/Timeline.tsx) |
